...
function Healthcare() {
  ...
  const { data: items, isLoading } = useListHealthcare({ query: { queryKey: getListHealthcareQueryKey() } });
  const createMutation = useCreateHealthcare();
  const updateMutation = useUpdateHealthcare();
  const deleteMutation = useDeleteHealthcare();
    ...
    if (editItem) {
      updateMutation.mutate({ id: editItem.id, data }, {
        onSuccess: () => {
          ...
          queryClient.invalidateQueries({ queryKey: getListHealthcareQueryKey() });
          setOpen(false);
          setEditItem(null);
        },
  ...
  return <AppLayout>
    ...
    setForm({ title: "", category: "information", content: "", contactInfo: "" });
        ...
        {isLoading ? <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            {(items ?? []).map((item) => <Card key={item.id} className={`flex flex-col ${isAdmin ? "cursor-pointer hover:border-primary/50" : ""}`} onClick={() => {
    if (isAdmin) {
      setEditItem({ id: item.id, title: item.title, category: item.category, content: item.content, contactInfo: item.contactInfo });
      setForm({ title: item.title ?? item.information ?? "", category: item.category ?? "information", content: item.content ?? item.services ?? "", contactInfo: item.contactInfo ?? "" });
      setOpen(true);
    }
  }}>
                <CardHeader>
                ...
                <CardContent className="flex-1 space-y-3">
                  ...
                  {item.contactInfo && <div className="flex items-center gap-2 text-sm font-medium">
